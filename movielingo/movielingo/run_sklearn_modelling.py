...
def run_grid_search_CV(X_train, y_train, results_filename):
    parameters = [
    {
    ...
    'clf__estimator': [GradientBoostingRegressor(), RandomForestRegressor()],
    ...
    grid_search = RandomizedSearchCV(benchmark_pipeline, parameters, cv=5,
      scoring = 'neg_mean_squared_error', verbose = 2,
      return_train_score = True, n_iter=1, n_jobs = 3)
...
if __name__ == '__main__':
   ...
   benchmark_pipeline = build_pipeline()
   benchmark_pipeline = fit_pipeline(benchmark_pipeline, X_train, y_train, 'results_file.txt')
   best_model = run_grid_search_CV(X_train, y_train, 'results_file.txt')
